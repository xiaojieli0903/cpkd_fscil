...
from mmfscil.apis.eval_hook import DistEvalHook, EvalHook
from mmfscil.apis.test_fn import multi_gpu_test, single_gpu_test
...
def train_model(model,
    ...
    if distributed:
        ...
        model = wrap_non_distributed_model(model,
                                           cfg.device,
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            logger.info(f'Trainable {name} ({torch.norm(param)})')
